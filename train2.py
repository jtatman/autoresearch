"""
Experiment 2: Reasoning-augmented function calling.

Changes from experiment 1 (train.py):
  - MAX_SEQ_LEN=2048: covers 23/25 eval examples with full prompt context
  - MICRO_BATCH=2, GRAD_ACCUM=4: keeps effective batch=8 within VRAM budget
  - xlam training pairs augmented with Claude-generated <think> blocks
  - lordx64/reasoning-distill-opus-4-7-max-sft mixed in as general reasoning signal
  - Custom evaluator handles <think>...</think> in generated output

Hypothesis: reasoning traces (mo data, mo reasoning) help the model learn
which function to call by making the selection logic explicit at training time.
"""
...
from prepare import (
    ...
    build_chat_messages, load_tokenizer, download_eval_data,
)
...
LEARNING_RATE  = 3e-4
WEIGHT_DECAY   = 0.01
GRAD_CLIP      = 1.0        # back to stable (0.5 added variance in exp1)
EPOCHS         = 60
WARMUP_FRAC    = 0.05
MICRO_BATCH    = 2          # halved: 2×2048 ≈ 4×1024 ≈ 8×512 activation budget
GRAD_ACCUM     = 4          # effective batch = 8
EVAL_BATCH     = 4          # smaller batches during eval (longer seqs)
MAX_SEQ_LEN    = 2048       # exp1=512; covers 23/25 eval prompts fully
MAX_NEW_TOKENS = 768        # budget for <think>+JSON during eval generation

# lordx64 dataset
N_LORDX64        = 200      # examples to draw from lordx64 (tunable)
MIN_THINK_TOKENS = 50       # minimum think block size to keep

# Eval indices that are "hard" (>512 tok at exp1; most now get full context)
_HARD_EVAL_INDICES = {0, 3, 5, 6, 7, 9, 10, 12, 13, 15, 16, 17, 18, 20, 21, 22, 23, 24}
_HARD_OVERSAMPLE   = 4

# Cache paths
THINK_CACHE    = os.path.join(CACHE_DIR, "xlam_think_blocks.json")
LORDX64_CACHE  = os.path.join(CACHE_DIR, "lordx64_filtered.json")
LORA_BEST_DIR2 = os.path.join(CACHE_DIR, "best_lora_exp2")

# ---------------------------------------------------------------------------
# Tokenizer (loaded early for prompt building)
# ---------------------------------------------------------------------------
...
with open(os.path.join(CACHE_DIR, "eval_data.json")) as _f:
    _eval_examples = json.load(_f)[:25]

# ---------------------------------------------------------------------------
# Synthetic training pairs (same 45 as experiment 1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Think block generation via Claude API (cached)
# ---------------------------------------------------------------------------

_THINK_GEN_SYSTEM = (
    "You generate chain-of-thought reasoning for function-calling training data. "
    "Given a scenario with a known correct answer, write a concise <think> block "
    "that reasons clearly from the user query to the function call. Focus on: "
    "what the user wants, which tool matches, and why those arguments. "
    "Be direct — 3-6 sentences is enough. Then reproduce the exact JSON answer."
)


def _think_prompt(tools, query, answer_json):
    tools_str = json.dumps(tools, ensure_ascii=False, indent=2)
    return (
        f"Available tools:\n{tools_str}\n\n"
        f"User query: {query}\n\n"
        f"Correct answer: {answer_json}\n\n"
...
def generate_xlam_think_blocks(eval_examples, pair_specs):
    """Generate and cache <think> blocks for all xlam training pairs."""
    if os.path.exists(THINK_CACHE):
        print(f"  Loaded cached think blocks from {THINK_CACHE}")
        with open(THINK_CACHE) as f:
            return json.load(f)
    ...
    results = {}
    total = len(eval_examples) + len(pair_specs)
    done  = 0

    def _call(key, tools, query, answer_json):
        nonlocal done
        try:
            msg = client.messages.create(
                model="claude-haiku-4-5",
                max_tokens=512,
                system=_THINK_GEN_SYSTEM,
                messages=[{"role": "user", "content": _think_prompt(tools, query, answer_json)}],
            )
            text = msg.content[0].text.strip()
            m = re.search(r"<think>(.*?)</think>", text, re.DOTALL)
            if m:
                results[key] = {"think": m.group(1).strip(), "answer": answer_json}
        except Exception as e:
            print(f"\n  [{key}] API error: {e}")
        done += 1
        print(f"\r  Generating think blocks: {done}/{total}", end="", flush=True)

    print(f"Generating think blocks for {total} xlam pairs via Claude API ...")
    for idx, ex in enumerate(eval_examples):
        _call(f"eval_{idx}", ex["tools"], ex["query"],
              json.dumps(ex["answers"], ensure_ascii=False))
    for i, (tools, query, answers) in enumerate(pair_specs):
        _call(f"synth_{i}", tools, query,
              json.dumps(answers, ensure_ascii=False))

    print(f"\n  Generated {len(results)}/{total} think blocks.")
    os.makedirs(CACHE_DIR, exist_ok=True)
    with open(THINK_CACHE, "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    return results


# ---------------------------------------------------------------------------
# lordx64 loading and filtering (cached)
# ---------------------------------------------------------------------------

def load_lordx64_data(tokenizer, n=N_LORDX64,
                      min_think_tokens=MIN_THINK_TOKENS,
                      max_tokens=MAX_SEQ_LEN):
    """
    Download, filter, and convert lordx64 reasoning examples.
    Returns list of {"prompt": str, "response": str} dicts.
    Cached to disk after first run.
    """
    if os.path.exists(LORDX64_CACHE):
        with open(LORDX64_CACHE) as f:
            all_pairs = json.load(f)
        print(f"  Loaded {len(all_pairs)} cached lordx64 pairs; using {min(n, len(all_pairs))}")
        import random; random.seed(42); random.shuffle(all_pairs)
        return all_pairs[:n]

    from datasets import load_dataset
    print("  Downloading lordx64/reasoning-distill-opus-4-7-max-sft ...")
    ds = load_dataset("lordx64/reasoning-distill-opus-4-7-max-sft", split="train")

    pairs = []
    stats = {"no_think": 0, "short_think": 0, "too_long": 0, "ok": 0}
    ...
    for row in ds:
        ...
        for part in parts[1:]:
            ...
            parsed[role.strip()] = content.replace("<|im_end|>", "").strip()

        user   = parsed.get("user", "")
        asst   = parsed.get("assistant", "")
        system = parsed.get("system", "You are a helpful AI assistant.")
        if not user or not asst:
            continue

        m = re.search(r"<think>(.*?)</think>", asst, re.DOTALL)
        if not m:
            stats["no_think"] += 1
            continue

        think_tok = len(tokenizer.encode(m.group(1), add_special_tokens=False))
        if think_tok < min_think_tokens:
            stats["short_think"] += 1
            continue
        ...
        total  = len(tokenizer.encode(
            prompt + asst + tokenizer.eos_token, add_special_tokens=False))

        if total > max_tokens:
            stats["too_long"] += 1
            continue

        pairs.append({"prompt": prompt, "response": asst})
        stats["ok"] += 1

    print(f"  lordx64: {stats['ok']} usable / {len(ds)} total "
          f"(dropped: {stats['no_think']} no-think, "
          f"{stats['short_think']} short-think, {stats['too_long']} too-long)")

    os.makedirs(CACHE_DIR, exist_ok=True)
    with open(LORDX64_CACHE, "w") as f:
...
print("\n[1/4] Generating / loading xlam think blocks ...")
_think_blocks = generate_xlam_think_blocks(_eval_examples, PAIR_SPECS)

# Eval pairs with think blocks (hard × HARD_OVERSAMPLE, easy × 1)
...
for idx, ex in enumerate(_eval_examples):
    ...
    prompt = _TOKENIZER.apply_chat_template(
        msgs, tokenize=False, add_generation_prompt=True)
    key    = f"eval_{idx}"
    if key in _think_blocks:
        tb = _think_blocks[key]
        response = f"<think>\n{tb['think']}\n</think>\n\n{tb['answer']}"
    else:
...
# Synthetic pairs with think blocks
...
XLAM_PAIRS = EVAL_PAIRS + SYNTH_PAIRS

print(f"\n[2/4] Loading lordx64 reasoning data ...")
_lordx64 = load_lordx64_data(_TOKENIZER, n=N_LORDX64)
LORDX64_PAIRS = [(d["prompt"], d["response"]) for d in _lordx64]

ALL_PAIRS = XLAM_PAIRS + LORDX64_PAIRS

_n_hard = len(_HARD_EVAL_INDICES)
_n_easy = len(_eval_examples) - _n_hard
print(f"\nTraining pairs: {len(XLAM_PAIRS)} xlam-with-think "
      f"({_n_hard} hard×{_HARD_OVERSAMPLE} + {_n_easy} easy×1 eval "
      f"+ {len(SYNTH_PAIRS)} synthetic) "
      f"+ {len(LORDX64_PAIRS)} lordx64 = {len(ALL_PAIRS)} total")

# ---------------------------------------------------------------------------
# SFT batch formatter (right-truncation, matching eval tokenizer behaviour)
# ---------------------------------------------------------------------------

def make_sft_batch_rt(pairs: list[tuple[str, str]], tokenizer,
                      max_length: int = MAX_SEQ_LEN):
    """SFT batch with right-truncation. Prioritises keeping the response."""
    ...
    for prompt, response in pairs:
        ...
        all_labels.append([-100] * len(p_ids) + list(r_ids))
...
# ---------------------------------------------------------------------------
# Evaluator: think-block-aware (replaces prepare.py's evaluate_function_calling)
# ---------------------------------------------------------------------------

@torch.no_grad()
def evaluate_with_thinking(model, tokenizer,
                            batch_size: int = EVAL_BATCH,
                            device: str = "cuda") -> dict:
    """
    Evaluate function-calling accuracy on the fixed 25-example eval set.
    Strips <think>...</think> blocks from model output before JSON parsing.
    Primary metric: function name exact match.
    """
    examples = download_eval_data()[:25]
    model.eval()
    ...
    for start in range(0, n_total, batch_size):
        ...
        for i, ex in enumerate(batch):
            ...
            if think_m:
                text = text[think_m.end():].strip()
                n_with_think += 1

            # JSON parse (identical logic to prepare.py)
            try:
                if "```" in text:
                    text = text.split("```")[1]
                ...
                if bracket >= 0 or brace >= 0:
                    si = (bracket if brace < 0
                          else (brace if bracket < 0
                                else min(bracket, brace)))
                    text = text[si:]
                ...
                gt_names   = [a["name"] for a in ex["answers"]
    ...
    return {
        ...
        "think_rate":     n_with_think / n_total if n_total else 0.0,
        "n":              n_total,
    }


# ---------------------------------------------------------------------------
# Load model and apply fresh LoRA
# ---------------------------------------------------------------------------

print("\n[3/4] Loading base model and applying LoRA ...")
t_start = time.time()
...
# ---------------------------------------------------------------------------
# Pre-tokenise all training pairs (done once, reused across epochs)
# ---------------------------------------------------------------------------

print(f"\n[4/4] Pre-tokenising {len(ALL_PAIRS)} training pairs ...")
batch_data = make_sft_batch_rt(ALL_PAIRS, _TOKENIZER)
print(f"  Batch shape: {batch_data['input_ids'].shape}  "
      f"(max_seq={batch_data['input_ids'].shape[1]} tokens)")
...
print(f"\n--- Fine-tuning: {EPOCHS} epochs, {len(ALL_PAIRS)} pairs, "
...
for epoch in range(1, EPOCHS + 1):
    ...
    for bs in range(0, len(indices), MICRO_BATCH):
        ...
        print(f"\repoch {epoch}/{EPOCHS} step {completed} | "
...
print("\n--- Post-training evaluation ---")
model.eval()
res = evaluate_with_thinking(model, _TOKENIZER, batch_size=EVAL_BATCH)
print(f"name_accuracy : {res['name_accuracy']:.4f}  "
      f"({int(res['name_accuracy'] * res['n'])}/{res['n']})")
print(f"parse_rate    : {res['parse_rate']:.4f}")
print(f"think_rate    : {res['think_rate']:.4f}  "
      f"(fraction of outputs that included a <think> block)")
...
best_path  = os.path.join(LORA_BEST_DIR2, "best_accuracy.txt")
best_saved = 0.0
...
if res["name_accuracy"] > best_saved:
    print(f"New best ({res['name_accuracy']:.4f} > {best_saved:.4f}) "
          f"— saving to {LORA_BEST_DIR2}")
    os.makedirs(LORA_BEST_DIR2, exist_ok=True)
    model.save_pretrained(LORA_BEST_DIR2)
    _TOKENIZER.save_pretrained(LORA_BEST_DIR2)
...
print(f"think_rate:       {res['think_rate']:.6f}")
print(f"training_seconds: {time.time() - t_train_start:.1f}")
print(f"total_seconds:    {t_end - t_start:.1f}")
print(f"peak_vram_mb:     {peak_vram_mb:.1f}")
print(f"completed_steps:  {completed}")
print(f"epochs:           {epoch}")
print(f"xlam_pairs:       {len(XLAM_PAIRS)}")
print(f"lordx64_pairs:    {len(LORDX64_PAIRS)}")
